...
public class MembersController extends AbstractCustomController {
    ...
    private class ActionCell extends TableCell<MemberDto, Boolean> {
        ...
        ActionCell() {
            ...
            ImageView updtateView = new ImageView(new Image("/img/delete_32.png"));
            hBox.getChildren().add(updtateView);
            updtateView.setOnMouseReleased(event -> {
                table.getSelectionModel().select(getTableRow().getIndex());
                addMember(event);
            });

            ImageView deleteView = new ImageView(new Image("/img/delete_32.png"));

            hBox.getChildren().add(deleteView);
            deleteView.setOnMouseReleased(event -> showYesNoDialog("todo", o -> removeMember()));
        }
